...
export function isDuplicateChoice(text, choices, excludeIndex = -1) {
  ...
  return choices.some(
    (choice, index) => index !== excludeIndex && choice.text.toLowerCase() === normalizedText
  );
}
...
export function getValidColor(index) {
  ...
  const safeIndex = typeof index === 'number' && !isNaN(index) ? Math.max(0, index) : 0;
...
export function generateWheelSlices(wheelData) {
  ...
  const totalSlices = wheelData.reduce((total, item) => total + item.occurrences, 0);
