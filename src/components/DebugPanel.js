  ...
  return (
    <>
      ...
      >
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-col gap-3 bg-white rounded-lg p-3 border border-slate-200">
              {computationSteps.map((step, index) => (
                <div
                  key={index}
                  className="flex flex-col gap-1 p-2 rounded bg-slate-50 border-l-3 border-l-sky-500"
                >
                  ...
                  <div className="text-xs text-slate-500 italic">{step.description}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Slice Positions */}
        <div className="mb-4">
          <h4 className="text-base font-medium text-slate-600 mb-2">
            ...
            {slicePositions.map(data => (
              ...
              >
                ...
                <div className="flex justify-between items-center">
                  <span>
                    Position: {data.startAngle}° → {data.endAngle}°
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span>At indicator (0°):</span>
                  <span
                    className={`font-semibold ${data.containsZeroDegree ? 'text-green-500' : 'text-red-500'}`}
                  >
        ...
        <div className="bg-slate-100 rounded-lg p-3 mb-4 text-sm leading-relaxed">
          ...
          <p className="mb-2">
            <strong>Computation:</strong> For each slice, we calculate its position after rotation
            by subtracting the normalized rotation from its original position. This is because when
            the wheel rotates clockwise (positive rotation), the slices move backward relative to
            the fixed indicator.
          </p>
          <p className="m-0">
            <strong>Direction consistency:</strong> Both the visual animation and the mathematical
            computation use the same direction convention: positive rotation values = clockwise
            rotation.
          </p>
        </div>
      </div>
